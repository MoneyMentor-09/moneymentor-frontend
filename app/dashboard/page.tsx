...
export default function DashboardPage() {
  ...
  // Placeholder previous-month comparisons
  const previousMonthIncome = totalIncome > 0 ? totalIncome * 0.9 : 100;
  const previousMonthExpenses =
    totalExpenses > 0 ? totalExpenses * 1.1 : 100;
  ...
  const lineChartData = Object.entries(monthlyData).map(
    ([month, data]) => ({
      month,
      income: data.income,
      ...
      <DashboardLayout>
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            Please run setup scripts to initialize the database.
          </p>
          <Button asChild>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {expenseChange <= 0 ? (
                  <span className="text-green-600 flex items-center">
                    <ArrowUpRight className="h-3 w-3 mr-1" />
                    {Math.abs(expenseChange).toFixed(1)}% decrease
                  </span>
                ) : (
                  <span className="text-red-600 flex items-center">
                    <ArrowDownRight className="h-3 w-3 mr-1" />
        ...
        {alerts.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent security alerts and risk assessments
              </CardDescription>
            </CardHeader>
            <CardContent>
